...
- Запускает команды потоков (`/sdd`, `/ddd`, `/vdd`, `/tdd`) **в субагенте**
- Инжектирует релевантный ADR контекст в каждую сессию субагента
...
**Как работает:**
- Документирует все потоки до фазы PLAN approved
- Компилирует approved планы в архитектурные слои (0/1/2)
- Обнаруживает и разрешает gaps в исходных потоках
- Запускает реализацию **в субагенте** для bottom-up выполнения (Layer 0 → Layer 1 → Layer 2)
