# Roadmap Dependencies Graph

## Overview

Полный dependency graph. Critical path highlighted для DFS execution.

## Current Goal

[none] | MVP | "[goal]"

## Dependency Graph

```
Legend:
  ════  Critical path (will be executed)
  ────  Other dependencies (skipped)
  ●     Complete
  ◐     In progress
  ○     Pending
  ∅     Skipped

[flow-a] ══════> [flow-b] ══════> [GOAL]
                     │
    [flow-c] ────────┘ (skipped, не на пути)
```

## Critical Path (DFS порядок)

| Order | Flow | Type | Status | Blocks |
|-------|------|------|--------|--------|
| 1 | - | - | - | - |

## All Flows

### On Critical Path

| Flow | Type | Phase | Status | Blocked By |
|------|------|-------|--------|------------|
| - | - | - | - | - |

### Not on Path (Skipped)

| Flow | Type | Why Skipped |
|------|------|-------------|
| - | - | - |

## ADRs (Read-only context)

| ADR | Type | Status | Affects Critical Path |
|-----|------|--------|----------------------|
| - | - | - | - |

## Dependency Details

### [flow-name]

**On critical path:** yes/no

**Blocked by:**
- [other-flow]: [reason]

**Blocks:**
- [another-flow]: [reason]

**Related ADRs:**
- ADR-NNN: [как это constrain/enable]

---

*Auto-generated /roadmap. Manual edits будут overwritten.*
